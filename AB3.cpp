...
class Point {
	...
	/*	long area = (P2->x - P0->x)*(P1->y - P0->y) - 
			    (P1->x - P0->x)*(P2->y - P0->y); */
		//Nov 30 change
                double  area = (double)(P1->x - P0->x)* (double)(P2->y - P0->y) -
...
double  Segment:: Slope(flag *start, flag *end){
	if(start->P->x != end->P->x){
		double slope = (double)(end->P->y - start->P->y)/(double)(end->P->x - start->P->x);
		return slope;
	}else{
...
struct EventLess{
	bool operator()(Segment *Seg1, Segment *Seg2){
		...
		if((Seg1->start->P->Equal(Seg2->start->P))&&(Seg1->end->P->Equal(Seg2->end->P))){
			return compareflags(Seg1->start,Seg2->start);
		}
		if( Seg1->start->P->x < Seg2->start->P->x ){
		//	//cout<<"Seg1x<seg2x:"<<Segment::Side(Seg1,Seg2->start->P)<<endl;
			return (Segment::Side(Seg1,Seg2->start->P) > 0); // Nov30
		}else if(Seg1->start->P->x > Seg2->start->P->x ){
		//	//cout<<"Seg2x>Seg1x:"<<Segment::Side(Seg1,Seg2->start->P)<<endl;
			return (Segment::Side(Seg2,Seg1->start->P) < 0); //Nov 30
		}else if(Seg1->start->P->y != Seg2->start->P->y){
		//	cout<<"y:"<<Seg1->start->P->y<<"? "<<Seg2->start->P->y<<endl;
			return Seg1->start->P->y < Seg2->start->P->y;
		}else{ // if(Segment:: Slope(Seg1->start,Seg1->end)!= Segment:: Slope(Seg2->start,Seg2->end)){
		//	cout<<"Slope:"<<slope1<<"?"<<slope2<<endl;
			return Segment:: Slope(Seg1->start,Seg1->end)<
				Segment:: Slope(Seg2->start,Seg2->end); 
			// change to slope instead of -slope Nov. 29
		}/*else{
			return compareflags(Seg1->start,Seg2->start);
		}*/
	}
...
int main(int argc, char *argv[]){
	...
	// Tight Sentinel line for Red Segments: Red below and Red above 
 	...
 	Point *RBS = new Point(xmin- 5, ymin-5), *RBT= new Point(xmax+5, ymin-5);  // Red below start/terminal 
	Point *RAS = new Point(xmin- 5, ymax+5), *RAT= new Point(xmax+5, ymax+5);  // Red above start/terminal
	flag *RBSF= new flag(RBS, 0, 1, "R"), *RBTF=new flag(RBT, 0, 0, "R");  //Red Below start/terminal flag
	flag *RASF= new flag(RAS, m+1, 1, "R"), *RATF= new flag(RAT, m+1, 0, "R");   // Red above start/terminal flag
	Segment *Rsegbotom = new Segment(RBSF,RBTF), *Rsegtop= new Segment(RASF,RATF); //Red bottom/top sentinel line */
	
	Point *BBS= new Point(xmin-10, ymin-10), *BBT= new Point(xmax+10, ymin-10);  // Blue below start/terminal 
	Point *BAS= new Point(xmin-10, ymax+10), *BAT= new Point(xmax+10, ymax+10);  // Blue above start/terminal
	flag *BBSF= new flag(BBS, 0, 1, "B"), *BBTF= new flag(BBT, 0, 0, "B");  //Blue Below start/terminal flag
	flag *BASF = new flag(BAS, n+1, 1, "B"),*BATF= new flag(BAT, n+1, 0, "B");   //Blue above start/terminal flag
	Segment *Bsegbotom= new Segment(BBSF,BBTF), *Bsegtop=new Segment(BASF,BATF);  //Blue bottom/top sentinel line
...
*/
	...
	delete Bsegtop->start->P;
	delete Bsegtop->end->P;
	delete Bsegtop->start;
	delete Bsegtop->end;
	delete Bsegtop;

	delete Bsegbotom->start->P;
	delete Bsegbotom->end->P;
	delete Bsegbotom->start;
	delete Bsegbotom->end;
	delete Bsegbotom;

	delete Rsegtop->start->P;
	delete Rsegtop->end->P;
	delete Rsegtop->start;
	delete Rsegtop->end; 
	delete Rsegtop;
	
	delete Rsegbotom->start->P; 
	delete Rsegbotom->end->P;
	delete Rsegbotom->start;
	delete Rsegbotom->end;
	delete Rsegbotom;
